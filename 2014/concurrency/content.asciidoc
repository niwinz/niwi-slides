...
- Jetty/Netty implements proactor pattern: handles events asynchronously and executes handlers also asynchronously using threads. (concurrency and paralellism)
...
Previously functions are pure and can be composed, enabling concurrency and paralellism:
...
Almos all lenguages implements futures.
