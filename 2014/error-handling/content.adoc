...
[options="step"]
====
- It looks better and can be much less verbose that the previous version. 
...
- Other languages also can use that but much less expressivenes (can be looks ugly).
...
- It has the advantage that the composition is defined on type (you should not reapetly define different
  ways of compose different errors).
- You can create own types and mix them in the same pipeline.

<<<

Also, it works better with async/streams flows (ex core.async).  Suppose that validator functions
return channels instead of values.

[source, clojure]
----
(defn clean-contact [params]
  (go
    (m/>> (<! (clean-email params))
          (<! (clean-phone params))
          (<! (clean-state params))
          (either/right params))))
----
...
Example of ring handler that uses monadic error handling.

[source, clojure]
----
(defn document-revs-list
  [{:keys [params] :as req}]
  (m/mlet [:let   [dbname   (:dbname params)
                   collname (:collname params)
                   docid (:docid params)]
           result (api/get-revisions-of dbname collname docid)]
    (m/return (ok (map s/to-plain-object result)))))
----

<<<

Ring middleware used for convert responses from handlers that returns
monads.

[source, clojure]
----
(defn wrap-monadic-ctrl
  [handler]
  (fn [req]
    (let [response (handler req)]
      (cond
       (either/left? response)
       (left-as-response response)

       (either/right? response)
       (either/from-either response)))))
----
